...
class ManualUntestedDieIn(BaseModel):
    wafer: str = Field(min_length=1)
    untested: bool
    user_name: str = "operator"


class LayoutViewSettingsIn(BaseModel):
    wafer_scale: float = Field(default=1.0, ge=0.5, le=1.5, allow_inf_nan=False)
    wafer_offset_x: float = Field(default=0.0, ge=-3.0, le=3.0, allow_inf_nan=False)
    wafer_offset_y: float = Field(default=0.0, ge=-3.0, le=3.0, allow_inf_nan=False)


class LayoutTemplateRenameIn(BaseModel):
...
async def api_layout_templates() -> dict[str, Any]:
    return {"templates": layout_store.list_layout_templates()}


@app.put("/api/layout/templates/{layout_id}/select")
async def api_select_layout_template(layout_id: str) -> dict[str, Any]:
    try:
        layout = layout_store.select_layout_template(layout_id)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    store.add_audit("layout_template_select", {"layout_id": layout_id})
    return {"ok": True, "layout": layout_store.public_layout_info(layout)}


@app.put("/api/layout/templates/{layout_id}/rename")
async def api_rename_layout_template(layout_id: str, body: LayoutTemplateRenameIn) -> dict[str, Any]:
    try:
        layout = layout_store.rename_layout_template(layout_id, body.name)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    store.add_audit("layout_template_rename", {"layout_id": layout_id, "name": body.name.strip()})
    return {"ok": True, "layout": layout_store.public_layout_info(layout)}


@app.delete("/api/layout/templates/{layout_id}")
async def api_delete_layout_template(layout_id: str) -> dict[str, Any]:
    try:
        current = layout_store.delete_layout_template(layout_id)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    store.add_audit("layout_template_delete", {"layout_id": layout_id})
    return {"ok": True, "layout": layout_store.public_layout_info(current) if current else None}


@app.get("/api/layout/view-settings/{layout_id}")
async def api_layout_view_settings(layout_id: str) -> dict[str, Any]:
    if not layout_id.strip():
        raise HTTPException(status_code=422, detail="layout_id 不能为空")
    return store.get_layout_view_settings(layout_id)


@app.put("/api/layout/view-settings/{layout_id}")
async def api_save_layout_view_settings(
    layout_id: str, body: LayoutViewSettingsIn
) -> dict[str, Any]:
    if not layout_id.strip():
        raise HTTPException(status_code=422, detail="layout_id 不能为空")
    saved = store.save_layout_view_settings(layout_id, body.model_dump())
    store.add_audit("layout_view_settings_save", saved)
...
@app.put("/api/dies/{die_id}/manual-untested")
async def api_set_die_manual_untested(
    die_id: str,
    body: ManualUntestedDieIn,
) -> dict[str, Any]:
    """人工排除/恢复某颗 Die；下次判定自动应用并重算全部统计。"""
    store.set_die_manual_untested(body.wafer, die_id, body.untested)
    store.add_audit(
        "die_manual_untested" if body.untested else "die_restore_tested",
        {"wafer": body.wafer, "die_id": die_id},
        user_name=body.user_name,
    )
    return {"ok": True, "wafer": body.wafer, "die_id": die_id, "manual_untested": body.untested}
...
async def api_judge(body: JudgeIn) -> dict[str, Any]:
    try:
        result = await judge_wafer(
            ...
            manual_untested_die_ids=store.list_manual_untested_die_ids(body.wafer),
        )
        store.add_audit(
            "judge",
            {
