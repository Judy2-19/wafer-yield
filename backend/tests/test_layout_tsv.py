...
class LayoutTsvTests(unittest.TestCase):
    ...
    def test_same_layout_content_keeps_a_stable_layout_id(self) -> None:
        text = SF_DR8.read_text(encoding="utf-8")
        first = layout_store.layout_id_for_text(text)
        second = layout_store.layout_id_for_text("\ufeff" + text.replace("\r\n", "\n"))
        changed = layout_store.layout_id_for_text(text.replace("\t41\n", "\t40\n", 1))
