...
def build_judge_workbook(payload: dict[str, Any]) -> bytes:
    ...
    for die in payload.get("dies") or []:
        manual_untested = die.get("manual_untested") is True
        fails = [
            r["name"]
        ...
        ws2.append(
            [
                ...
                die.get("y"),
                die.get("create_time"),
                "未测试" if manual_untested else "Pass" if die.get("pass") else "Fail",
                ",".join(fails),
            ]
        )
    ...
    for die in payload.get("dies") or []:
        if die.get("manual_untested") or die.get("pass"):
            continue
        for r in die.get("param_rows") or []:
